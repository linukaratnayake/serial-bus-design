...
module system_top_with_uart_bridge_dual_init_tb;
    ...
    localparam int unsigned UART_CLOCK_DIV = 4;
    ...
    // Bus A split-target wiring towards UART bridge.
    ...
    // UART serial links between the two FPGA halves.
    logic uart_a2b;
    logic uart_b2a;
    ...
    initial begin
        ...
        if (init1_read_data === INIT1_DATA && init2_read_data === INIT2_DATA && init1_ack_seen && init2_ack_seen && init1_split_ack_seen && !init2_split_ack_seen && b_split_target_last_write === INIT1_DATA && b_target1_last_write === INIT2_DATA)
            $display("[%0t] Dual-initiator UART bridge test PASSED", $time);
        else
            $display("[%0t] Dual-initiator UART bridge test completed with issues", $time);
    ...
    initial begin
        #400000;
        $fatal(1, "Timeout waiting for dual-initiator UART bridge test to complete");
    end
    ...
    // UART-based bridge target half (FPGA A).
    bus_bridge_target_uart #(
        .CLOCK_DIV(UART_CLOCK_DIV),
    ...
    // UART-based bridge initiator half (FPGA B).
    bus_bridge_initiator_uart #(
        .CLOCK_DIV(UART_CLOCK_DIV)
    ) u_bridge_initiator_uart (
        .clk(clk),
        .rst_n(rst_n),
